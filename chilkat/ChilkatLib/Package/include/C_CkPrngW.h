...
CK_VISIBLE_PUBLIC BOOL CkPrngW_GetEntropyBytes(HCkPrngW cHandle, int numBytes, HCkByteData outBytes);
CK_VISIBLE_PUBLIC BOOL CkPrngW_ImportEntropy(HCkPrngW cHandle, const wchar_t *entropy);
CK_VISIBLE_PUBLIC int CkPrngW_RandomInt(HCkPrngW cHandle, int low, int high);
CK_VISIBLE_PUBLIC BOOL CkPrngW_RandomPassword(HCkPrngW cHandle, int length, BOOL mustIncludeDigit, BOOL upperAndLowercase, const wchar_t *mustHaveOneOf, const wchar_t *excludeChars, HCkString outStr);
CK_VISIBLE_PUBLIC const wchar_t *CkPrngW_randomPassword(HCkPrngW cHandle, int length, BOOL mustIncludeDigit, BOOL upperAndLowercase, const wchar_t *mustHaveOneOf, const wchar_t *excludeChars);
CK_VISIBLE_PUBLIC BOOL CkPrngW_RandomString(HCkPrngW cHandle, int length, BOOL bDigits, BOOL bLower, BOOL bUpper, HCkString outStr);
CK_VISIBLE_PUBLIC const wchar_t *CkPrngW_randomString(HCkPrngW cHandle, int length, BOOL bDigits, BOOL bLower, BOOL bUpper);
CK_VISIBLE_PUBLIC BOOL CkPrngW_SaveLastError(HCkPrngW cHandle, const wchar_t *path);
#endif
